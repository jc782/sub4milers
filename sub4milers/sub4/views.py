...
def viewSub4(request):
    ...
    athletes = AthleteSub4.objects.all().annotate(row_number=Window(expression=RowNumber())).annotate(
    first_Date=Coalesce('firstDate', Value(the_past))).order_by('first_Date')
    context = {}
